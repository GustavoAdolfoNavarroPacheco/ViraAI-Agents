# -*- coding: utf-8 -*-
"""v3 assembly for YouTube horizontal.

Keeps the previous EDL, but places the camera PiP away from the bottom subtitle
zone. Final subtitles are expected in the bottom-center lower third.
"""
...
GRADE_CAM = "eq=contrast=1.06:saturation=1.04"
...
# Keep PiP clear of bottom-centered subtitles.
PANEL_X = 1920 - PANEL_W - 44
PANEL_Y = 220
CAM_X = PANEL_X + 22
CAM_Y = PANEL_Y + 42
...
def render_pip():
    for seg in EDL["segments"]:
        if seg["layout"] != "pip":
            continue
        run(build_pip(seg, seg_path(seg)), f"{seg['id']} (pip v3) {seg['end']-seg['start']:.2f}s")
...
def concat_copy(seg_list, out):
    ...
    run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", txt,
         "-c", "copy", "-movflags", "+faststart", out], f"concat -> {os.path.basename(out)}")
...
def assemble():
    ...
    base = os.path.join(HERE, "base_v3.mp4")
    run(["ffmpeg", "-y", "-i", pA, "-i", pB, "-i", pC,
         "-filter_complex", fc,
         "-map", "[outv]", "-map", "[outa]",
         "-r", str(FPS), "-vsync", "cfr",
         *QSV, *AENC, "-movflags", "+faststart", base], "xfade assemble -> base_v3.mp4")
    print(f"DONE base_v3.mp4 total={probe(base):.3f}s")
